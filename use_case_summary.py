...
# Tentukan path ke file .env
    # dotenv_path = r"D:\Randy\MSIB\pyLat\assignment-5\.env"

    # Memuat file .env dengan path yang ditentukan
    load_dotenv()

    # Update variabel global yang akan digunakan untuk autentikasi di fungsi lain
    globals()["api_key"] = os.getenv("api_key", None)
    globals()["watsonx_project_id"] = os.getenv("project_id", None)
    
    # Verifikasi apakah api_key dan project_id telah dimuat
    if globals()["api_key"] and globals()["watsonx_project_id"]:
        print("API key and project ID loaded successfully.")
        
    else:
        print("Error: Could not load API key or project ID.")
